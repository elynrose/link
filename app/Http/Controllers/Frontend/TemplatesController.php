...
class TemplatesController extends Controller
{
    ...
    public function store(StoreTemplateRequest $request)
    {
        ...
        if ($request->input('background_image', false)) {
            $template->addMedia(storage_path('tmp/uploads/' . basename($request->input('background_image'))))->toMediaCollection('background_image');
        }
    ...
    public function update(UpdateTemplateRequest $request, Template $template)
    {
        ...
        if ($request->input('background_image', false)) {
            if (! $template->background_image || $request->input('background_image') !== $template->background_image->file_name) {
                if ($template->background_image) {
                    $template->background_image->delete();
                }
                $template->addMedia(storage_path('tmp/uploads/' . basename($request->input('background_image'))))->toMediaCollection('background_image');
            }
