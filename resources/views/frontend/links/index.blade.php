        ...
        <div class="col-md-12">
            @can('link_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.links.create') }}">
                            ...
                            <tbody>
                                @foreach($links as $key => $link)
                                    <tr data-entry-id="{{ $link->id }}">
                                        ...
                                        <td>
                                            <span style="display:none">{{ $link->add_utm ?? '' }}</span>
                                        ...
                                        <td>
                                            @can('link_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.links.show', $link->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('link_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.links.edit', $link->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('link_delete')
                                                <form action="{{ route('frontend.links.destroy', $link->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('link_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.links.massDestroy') }}",
